...
- **2026-09-18：仓库已上传 GitHub 供组员 clone 协同**（`fapig1/ZQXNXS`，提交 `30d72ec`）。`.gitignore` 排除：Vuforia 本地包 `ARPet/Packages/*.tgz`（132 MB 超 GitHub 单文件 100 MB 硬限制，组员按 `ARPet/SETUP.md` 第 5 节自行导入 unitypackage 获得）、`ArtSource/Characters/Pet01/Archive/`（182 MB 历史归档，仅本机保留）、`ARPet/.utmp/`、`*.npz`。入库总量约 141 MB、476 文件，最大单文件 17.6 MB。本机到 GitHub 的 HTTPS 大推送会被重置、SSH 22 端口超时，已改走 `ssh.github.com:443`（`~/.ssh/config` 已配 `Host github.com` 块，remote 已切为 SSH 地址）。
