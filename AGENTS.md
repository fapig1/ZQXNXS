...
项目事实与范围以同目录《开题说明_AR萌宠互动系统.md》、实际工程状态和用户最新指示为依据。本文为 2026-09-08 的开发约定与状态快照，后续完成工作时同步更新，不将旧快照当作永久限制。

- 开发者 1 人，约 4 周，有一定 Unity 基础。
- 目标平台为 Android 手机和平板，首要实测设备是 iQOO Z9 Turbo / Android 15。
- 当前阶段为开题文档、参考素材核查与开发环境核对；本目录尚无 Unity 工程、Blender 角色源模型、FBX、APK 或真机测试结果。
...
Android 应用主体在 Unity 工程中开发并打包；Android Studio / adb 用于调试，原生插件按实际需要接入。Unity 6 LTS 已确定，待锁定的是具体 Unity 发行版与补丁版本、Vuforia SDK 版本和渲染管线；本次网络限制使官方支持网页未能实时复核，不将历史 Vuforia 9.0 材料作为当前版本依据。

## 用户已明确的工作方式

- 用户计划由助手直接调用 D:/blender.exe 与 Python/bpy 制作、检查和导出资产；不使用用户已要求忽略的 blender-asset-package skill。2026-09-08 已跑通后台 bpy 调用并确认 FBX 导出接口可用，尚未执行模型生成或 FBX 导出。
...
以下为后续建议，除已有文件外不表示目录已经创建。源资产、Unity 导入资源、可选 Android 插件与打印标记的详细分工见 [技术栈与开发环境](Docs/技术栈与开发环境.md)：
...
- 用户于 2026-09-08 选择 `ArtSource/References/20260908_naiwa_assets/milkyfrog_official/milkyfrog-hero.png` 的站立主视觉作为奶蛙建模基准。其他图补充动作和表情，不逐项覆盖主图的身体比例、腿长、腹部轮廓和五官位置。
